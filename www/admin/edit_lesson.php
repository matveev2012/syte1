...
<table width="900" height="489" border="0px" align="center" cellpadding="0" cellspacing="0" class="main_border" bgcolor="#FFFFFF">
  ...
  if(!isset($id)){
  $result=mysql_query("SELECT title,id FROM lesson");
  $myrow=mysql_fetch_array($result);
  do
  {
	printf("<p><a href='edit_lesson.php?id=%s'>%s</a> ",$myrow["id"],$myrow["title"]);	
  }
...
$result=mysql_query("SELECT * FROM lesson WHERE id=$id");
$myrow=mysql_fetch_array($result);
print<<<HERE
 <form action="update_lesson.php" method="post" name="form1" id="form1">
